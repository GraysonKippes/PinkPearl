...
bool area_get_room_ptr(const area_t area, const Offset room_position, const Room **room_pptr) {
	...
	if (area.positions_to_rooms == nullptr) {
		logMsg(LOG_LEVEL_ERROR, "Error getting room pointer: area.positions_to_rooms is nullptr.");
		return false;
	}
	...
	if (room_position_index < 0) {
		logMsgF(LOG_LEVEL_ERROR, "Error getting room pointer: room position index is negative (%i).", room_position_index);
		return false;
	}
	...
	if (room_index >= (int)area.num_rooms) {
		logMsgF(LOG_LEVEL_ERROR, "Error getting room pointer: room index (%i) is not less than total number of rooms (%i).", room_index, (int)area.num_rooms);
		return false;
	}
...
int area_get_room_index(const area_t area, const Offset room_position) {
	...
	if (area.positions_to_rooms == nullptr) {
		logMsg(LOG_LEVEL_ERROR, "Error getting room index: area.positions_to_rooms is nullptr.");
		return -2;
	}
