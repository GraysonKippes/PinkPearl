...
binary_semaphore_t create_binary_semaphore(const VkDevice vk_device) {
	...
	vkCreateSemaphore(vk_device, &semaphore_create_info, NULL, &binary_semaphore.semaphore);
	return binary_semaphore;
}

bool destroy_binary_semaphore(binary_semaphore_t *const binary_semaphore_ptr) {

	if (binary_semaphore_ptr == NULL) {
		return false;
	}

	vkDestroySemaphore(binary_semaphore_ptr->device, binary_semaphore_ptr->semaphore, NULL);
...
timeline_semaphore_t create_timeline_semaphore(const VkDevice vk_device) {
	...
	vkCreateSemaphore(vk_device, &semaphore_create_info, NULL, &timeline_semaphore.semaphore);
	return timeline_semaphore;
}

bool destroy_timeline_semaphore(timeline_semaphore_t *const timeline_semaphore_ptr) {

	if (timeline_semaphore_ptr == NULL) {
		return false;
	}

	vkDestroySemaphore(timeline_semaphore_ptr->device, timeline_semaphore_ptr->semaphore, NULL);
