...
/* TODO
 * add command pool
 * add (pre-recorded) command buffers
*/
...
void create_compute_pipelines(VkDevice logical_device, VkPipeline *compute_pipelines, uint32_t num_compute_shaders, ...) {
	...
	log_message(INFO, "Creating compute pipelines...");
	...
	for (uint32_t i = 0; i < num_compute_shaders; ++i) {
		...
		create_pipeline_layout(logical_device, compute_shader.m_descriptor_set_layout, &create_infos[i].layout);
	}
