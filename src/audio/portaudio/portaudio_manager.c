...
static int portaudio_stream_callback(const void *pInputBuffer, void *pOutputBuffer, unsigned long int frameCount, const PaStreamCallbackTimeInfo *pTimeInfo, PaStreamCallbackFlags statusFlags, void *pUserData) {
	(void)pInputBuffer;
	(void)pTimeInfo;
	(void)statusFlags;
	(void)pUserData;
...
void terminate_portaudio(void) {
	...
	const PaError stopStreamResult = Pa_StopStream(audio_stream);
	...
	const PaError closeStreamResult = Pa_CloseStream(audio_stream);
