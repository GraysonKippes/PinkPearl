...
struct material {
	...
	// Height and orientation of each texel.
	// R = position in the x axis, [0, 1] -> [scene extent left, scene extent right]
	// G = position in the y axis, [0, 1] -> [scene extent bottom, scene extent top]
	// B = position in the z axis, [0, 1] -> [scene extent far, scene extent near]
	// A = unused
	sampler2DArray position_map;

	// Height and orientation of each texel.
	// R = rotation about the x axis, [0, 1] -> [0, pi/2]
	// G = rotation about the y axis, [0, 1] -> [0, pi/2]
	// B = rotation about the z axis, [0, 1] -> [0, 2pi]
	// A = unused
	sampler2DArray rotation_map;

	// Specularity and emmissivity of each texel.
	sampler2DArray pbr_map;
};
